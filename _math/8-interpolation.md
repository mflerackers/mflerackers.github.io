...
We will start with a simple linear interpolation between two values. Let's say we have a spaceship which needs to rendezvous with a bigger ship to trade with. The bigger ship is stationery relatively to our ship, so we can just trace a straight line from our current position to our destination.
...
Instead of two points, we'll use three in our path. First we find a point from which we would approach the ship's docking bay, far enough so we don't intersect the ship. We first fly to this waypoint, and from there to the docking bay.
...
While this works, the artists are not going to like this solution. It's not realistic for a ship to fly straight angles as inertia won't allow for it. The ship would have to slow down on approach of the waypoint, turn, and accelerate again, which is a waste of fuel.
...
Let's imagine that the trajectory from b to c is another ship who's destination is also the docking port. We'll use as our path no longer a to b, but a to p, the position of the other ship at that moment in time. The former ship had a trajectory which was a line. The trajectory of our ship is still a line at each point in time, however the destination for the line is now different at each moment in time. Our trajectory across time is no longer a line.
...
Our path is a second order polynomial which goes from a to c and is pulled towards b. What we did is create a spline motion or a curve.
...
But as you can see, there is no hard math behind it, it is merely a linear blend on top of two linear blends.
...
But isn't this a step back? At least we had only points before, we just didn't know where to place them. Now we have tangent vectors, but we are going to calculate these tangent vectors in function of points on our path. This way we will be able to define our path by the points our path needs to go through, no more parameters which influence our path in a way we don't have full control over.

### Catmull-Rom spline

A Catmull-Rom spline defines the tanget vectors as

$$
m_k=\frac{p_{k+1}-p_{k-1}}{t_{k+1}-t_{k-1}}
$$

In most even spacing is used, so $$t_{k+1}-t_{k-1}$$ can be set to 1 giving
